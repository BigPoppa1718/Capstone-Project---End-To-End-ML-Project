...
# Resolve absolute production output directories relative to project root
raw_output_path = os.path.join(PROJECT_ROOT, "data", "raw", "healthcare_raw.csv")
processed_output_path = os.path.join(PROJECT_ROOT, data_cfg["processed_data_input"])
...
provider_specialties = ["Cardiology", "Orthopedics", "Family Medicine", "Pediatrics", "Dermatology"]
...
cpt_code_map = {"99213": 100, "99214": 150, "93000": 250, "27447": 5000, "99204": 200}
...
def calculate_financials(row):
    ...
    allowed_amount = round(base_fee * row["service_weight"] * spec_mod * fac_mod * geo_mod * tier_mod, 2)
    ...
    return pd.Series([submitted_charges, allowed_amount, percent_of_charge, final_reimbursement, is_capped_by_charges, is_capped_by_policy])


df_raw[[
    "submitted_charges", "allowed_amount", "percent_of_charge", 
    "final_reimbursement_rate", "is_capped_by_charges", "is_capped_by_policy"
]] = df_raw.apply(calculate_financials, axis=1)

df_raw["target_budget"] = df_raw["final_reimbursement_rate"].apply(lambda x: round(x * np.random.uniform(0.85, 1.15), 2))
df_raw["exceeds_budget"] = (df_raw["final_reimbursement_rate"] > df_raw["target_budget"]).astype(int)

# Inject null artifacts representing typical administrative data gaps
...
# EXPORT RAW ARTIFACT TO AUDIT LAYER
df_raw.to_csv(raw_output_path, index=False)

# =====================================================================
# PREPROCESSING TRANSFORMATION BLOCK
# =====================================================================
...
# Define and export the preprocessor instance to make it importable by pytest
...
# Use a standard conditional main block to prevent execution during imports
if __name__ == "__main__":
    df_cleaned = df_raw.copy()
    df_cleaned[numerical_cols] = SimpleImputer(strategy="median").fit_transform(df_cleaned[numerical_cols])
    df_cleaned[categorical_cols] = SimpleImputer(strategy="most_frequent").fit_transform(df_cleaned[categorical_cols])
    ...
    df_categorical_encoded = pd.DataFrame(encoded_categorical, columns=encoder.get_feature_names_out(categorical_cols))
    ...
    # EXPORT PREPROCESSED MATRIX TO SECURED ENDPOINT
    df_preprocessed_master.to_csv(processed_output_path, index=False)

    print("Data exported successfully into standard folders!")
